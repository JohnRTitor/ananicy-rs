...
use ananicy_platform::cgroup::{CgroupInfo, CgroupVersion};
use ananicy_platform::cgroup::manager::{CgroupController, CgroupManager};
...
fn test_cgroup_manager_v2_owned_subtree() {
    ...
    assert!(child_path.is_some(), "Should allow creating child in Owned subtree");
...
fn test_cgroup_manager_v2_foreign_rejection() {
    ...
    assert!(child_path.is_none(), "Should reject structural modifications in Foreign subtree");
}
...
fn test_cgroup_manager_v2_write_files() {
    ...
    assert!(success, "Should successfully set cpu.weight in Owned subtree");
