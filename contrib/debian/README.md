...
- **Run locally:** every step of `debian/rules` was executed against a real
  ...
  `GNU_RELRO` segment;   `cargo test --locked --offline` passed, 0 failures;
  `override_dh_auto_install`'s `make install` and the three
...
- **Not run locally:** `dpkg-buildpackage` and `lintian` do not exist in the
  ...
  (`dh_installsystemd`, `dh_installdeb`'s handling of `.dirs`) was
...
| `/usr/share/doc/ananicy-rs/` | `README.Debian` only |
...
`debian/copyright` and `README.Debian` are the only documentation installed.
`README.md`, `CONTRIBUTING.md` and `docs/*.md` are not: they are written for
people working on the daemon rather than for people running it, so they stay in
the source package, where a packager or a contributor reads them, instead of
becoming files in `/usr/share/doc` that nothing on the system opens. There is
therefore no `debian/ananicy-rs.docs` and no `ananicy-rs-doc` binary package;
`dh_installdeb` still installs `README.Debian` and `debian/copyright` on its
own, from the source tree.
