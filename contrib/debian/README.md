...
`README.Debian` explains the resulting `lintian` tag.
...
- The `changelog` distribution is `unstable`. Change it if you are targeting
  `experimental`.
- No `debian/lintian-overrides`. `no-manual-page-for-binary` is expected and is
  explained in `README.Debian`; the file was not added because `lintian` was not
  available to confirm the tag's spelling and version.
